  ...
  return (
    <div className="glass-card rounded-xl overflow-hidden border border-white/5">
      <div className="p-4 md:p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div
            className={`relative aspect-video rounded-lg border-2 border-dashed transition-all cursor-pointer overflow-hidden ${
              seekerPreview ? "border-[#00ff88]/40" : "border-white/10 hover:border-white/20"
            }`}
          ...
          >
            {seekerPreview ? (
              <>
                ...
                <div className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded text-[9px] bg-black/70 text-white/60">
                  抓人者視角 · 空景
                </div>
                <button
                  onClick={(e) => { e.stopPropagation(); setSeekerFile(null); setSeekerPreview(null); }}
                  className="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/60 text-white/40 text-xs flex items-center justify-center hover:bg-red-500/60 hover:text-white"
                >
                  ✕
                </button>
              </>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                ...
                <div className="text-xs text-white/30 font-medium">上傳抓人者看到的空景.jpg</div>
                <div className="text-[10px] text-white/20 mt-1">遊戲內截圖，無人、無 UI</div>
              </div>
            )}
          ...
          <div
            className={`relative aspect-video rounded-lg border-2 border-dashed transition-all cursor-pointer overflow-hidden ${
              hiderPreview ? "border-[#00ff88]/40" : "border-white/10 hover:border-white/20"
            }`}
          ...
          >
            {hiderPreview ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                ...
                <div className="text-xs text-white/30 font-medium">上傳你著色隱身後的截圖.jpg</div>
                <div className="text-[10px] text-white/20 mt-1">遊戲內截圖，已融入背景</div>
              </div>
            )}
        ...
        <div
          className="relative w-full aspect-[16/7] rounded-lg overflow-hidden bg-[#0a0a1a] cursor-crosshair border border-white/10"
          onClick={(e) => {
        ...
        >
          ...
          {clickCoord && (
            <div className="absolute w-8 h-8 -translate-x-1/2 -translate-y-1/2 pointer-events-none" style={{ left: `${clickCoord.x}%`, top: `${clickCoord.y}%` }}>
              <div className="w-full h-full rounded-full border-2 border-[#00ff88] bg-[#00ff88]/30 animate-pulse" />
              <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-mono text-[#00ff88] bg-black/70 px-1.5 py-0.5 rounded">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="flex items-center gap-3 p-3 rounded-lg bg-white/5 text-xs">
            <span className="text-white/40">座標</span>
            <code className="font-mono text-[#00ff88]">{clickCoord ? `x: ${clickCoord.x}%  y: ${clickCoord.y}%` : "尚未標記"}</code>
            {clickCoord && (
              <button type="button" onClick={() => setClickCoord(null)} className="ml-auto text-white/30 hover:text-white/60 text-xs">重新標記</button>
            )}
          </div>
          ...
          <input
            ...
            className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-white/80 placeholder-white/30 outline-none focus:border-[#00ff88]/40"
            required
          />

          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-white/30 uppercase tracking-wider font-semibold">色彩配方（多色圖層）</span>
              <button type="button" onClick={addColorLayer} className="text-[10px] text-[#00ff88] hover:underline">+ 新增圖層</button>
            </div>
            {colorLayers.map((layer, i) => (
              <div key={i} className="flex items-center gap-2">
                <input type="text" placeholder="名稱" value={layer.label} onChange={(e) => updateColorLayer(i, "label", e.target.value)}
                  className="w-20 px-2 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white/70 placeholder-white/30 outline-none focus:border-[#00ff88]/40" />
                <input type="text" placeholder="#HEX" value={layer.hex} onChange={(e) => updateColorLayer(i, "hex", e.target.value)}
                  className="w-24 px-2 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs font-mono text-white/80 placeholder-white/30 outline-none focus:border-[#00ff88]/40" />
                ...
                {colorLayers.length > 1 && (
                  <button type="button" onClick={() => removeColorLayer(i)} className="text-white/20 hover:text-red-400 text-xs flex-shrink-0">✕</button>
                )}
              </div>
            ))}
          </div>
          ...
          <textarea placeholder="筆刷模式建議（例如：使用 3 號畫筆以 45 度斜角畫出木紋條紋）"
            ...
            className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-white/80 placeholder-white/30 outline-none focus:border-[#00ff88]/40 resize-none h-16"
          />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input type="text" placeholder="你的暱稱（顯示於貢獻者欄）" value={formData.discoveredBy}
              ...
              className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-white/80 placeholder-white/30 outline-none focus:border-[#00ff88]/40" required />
            <input type="url" placeholder="社群連結（Twitch / YouTube / X）" value={formData.discovererLink}
              ...
              className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-white/80 placeholder-white/30 outline-none focus:border-[#00ff88]/40" />
          </div>

          <textarea placeholder="躲藏策略描述..." value={formData.description}
            onChange={(e) => setFormData((p) => ({ ...p, description: e.target.value }))}
            className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-white/80 placeholder-white/30 outline-none focus:border-[#00ff88]/40 resize-none h-20"
          />

          <div className="flex items-center gap-3">
            <button type="submit" className="px-5 py-2.5 rounded-lg text-sm font-semibold bg-[#00ff88] text-black hover:bg-[#00ff70] transition-all">提交待審核</button>
            <div className="flex items-center gap-1.5 text-[10px] text-white/30">
              <span className="w-1.5 h-1.5 rounded-full bg-yellow-400/60" />提交後需管理員核准
            </div>
          </div>

          {submitted && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-yellow-400/10 border border-yellow-400/20 text-xs text-yellow-300/80">
